...
if ( ! class_exists( 'StagTools' ) ) :
	...
	class StagTools {
		...
		public function init() {
			 ...
			 * @deprecated 1.2
			 */
			if ( current_theme_supports( 'stag-portfolio' ) ) {
				include_once 'includes/post-type/portfolio.php';
			}
			if ( current_theme_supports( 'stag-slides' ) ) {
				include_once 'includes/post-type/slides.php';
			}
			if ( current_theme_supports( 'stag-team' ) ) {
				include_once 'includes/post-type/team.php';
			}
			if ( current_theme_supports( 'stag-testimonials' ) ) {
				include_once 'includes/post-type/testimonials.php';
			}
		...
		public function stag_contextual_help() {
			...
			$currentScreen->set_help_sidebar(
				'<p><strong>' . sprintf(
					__( 'For more information:', 'stag' ) . '</strong></p>' .
					/* translators: %s: Link to Github wiki. */
					'<p>' . sprintf( __( 'Visit the <a href="%s" target="_blank">documentation</a> on the Github.', 'stag' ), esc_url( 'https://github.com/mauryaratan/stagtools/wiki' ) )
				) . '</p>' .
				'<p>' . sprintf(
